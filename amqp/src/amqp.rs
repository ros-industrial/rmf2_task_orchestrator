...
impl AmqpConnection {
    pub async fn new(uri: &str) -> Result<Self, AmqpError> {
        let connection = Connection::connect(uri, ConnectionProperties::default())
            .await
            .expect("Connection error to AMQP broker! Consumer unable to establish connection!");
...
pub async fn run_consumer(
    ...
    channel
        .exchange_declare(
            &config.exchange,
            lapin::ExchangeKind::Fanout, // Will probably need to have to add the exchange kind to the config for mapping right now the default is topic exchange
    ...
    for (routing_key, handler) in router.handlers() {
        ...
        channel
            ...
            .await?;
        tracing::debug!("Route: {}, Queue {}", routing_key, queue_name);
...
pub async fn consumer_loop(mut consumer: Consumer, handler: HandlerFn, routing_key: String) {
...
/// Uses a shared background listener for all responses - truly non-blocking
#[derive(Clone)]
...
impl AmqpClient {
                        ...
                        if msg_type == Some("TaskStatus") {
                            if let Some(full_id) = msg_id {
                                ...
                                if status == Some("COMPLETED") || status == Some("ERROR") {
                                    ...
                                    if let Some(senders) = pending_guard.remove(task_id) {
    ...
    pub async fn request_response(
        ...
        self.await_response(rx, task_id).await
    }

    /// Wait for a response without publishing (for duplicate executions)
    pub async fn wait_for_response(&self, task_id: &str) -> Result<Vec<u8>, AmqpError> {
        let (tx, rx) = oneshot::channel();
        ...
        tracing::info!("Waiting for already-published task {}", task_id);
        self.await_response(rx, task_id).await
    }
